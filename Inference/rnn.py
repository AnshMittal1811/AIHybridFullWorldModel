...
class MDNRNN(object):
  ...
  def __init__(self, hps, reuse=False, gpu_mode=False):
    ...
    # Defining RNN Scope and if it is reusable
    with tf.variable_scope('mdn_rnn', reuse=reuse):
      if not gpu_mode:
          
        with tf.device('/cpu:0'):
            
          tf.logging.info('CPU Mode')
          self.g = tf.Graph()
  ...
  def build_model(self, hps):
    ...
    # determines the number of the gaussian mixtures that have been used
    KMIX = self.num_mixture
    
    # width of the input layer and output layer
    ...
    # Main RNN  Cell Function
    cell_fn = tf.contrib.rnn.LayerNormBasicLSTMCell
    
    # defining  rnn, input, output dropout
    ...
    # Used in LayerNormBasicLSTMCell
    use_layer_norm = False if self.hps.use_layer_norm == 0 else True
    
    # Defining if RNN will have an dropout
    ...
    # No else cond for input or output dropout as if there was one they will vbecome similar to RNN Cell
    ...
    # Getting the final cell after the dropout
    self.cell = cell
    
    self.sequence_lengths = LENGTH
    
    # Inwidth will be 35 parameters nd output 32 Input  = 32 + from CNNVAE + actions that can be taken (slow, accelerate, direction)
    ...
    # Defining input to give in the dynamic_rnn
    actual_input_x = self.input_x
    
    # Defining initial states which can be 0
    self.initial_state = cell.zero_state(batch_size=hps.batch_size, dtype=tf.float32)
    
    # Nout is 3 dimension having (32,5,3)
    NOUT = OUTWIDTH * KMIX * 3
    
    #Getting the Wx + b(bias) = Z (det output)
    with tf.variable_scope('RNN'):
      
      # Output weights and biases to be used in xw_plus_b function
      output_w = tf.get_variable("output_w", [self.hps.rnn_size, NOUT])
      output_b = tf.get_variable("output_b", [NOUT])
    
    # Getting the Deterministi values from the LSTM
    output, last_state = tf.nn.dynamic_rnn(cell,
                                           ...
                                           scope="RNN")
    # Building the MDN
    # Reshaping to 2D matrix from 3D matrix which was (100, 1000, 256)
    #100 is no. of batches given, 1000 is no. of parameters in env, 256 states
    output = tf.reshape(output, [-1, hps.rnn_size])
    
    # XW + B for MDN middle layers
    output = tf.nn.xw_plus_b(output, output_w, output_b)
    # reshaping previous layer to get (100000, 256)
    output = tf.reshape(output, [-1, KMIX * 3])
    
    # saving the final state to an output variable
    self.final_state = last_state
    
    # just a function inside a method of class to calculate mdn coefficient
    def get_mdn_coef(output):
      # Slice the output, Z into 3 values along the horizontal value to get mean, log of mixtures and standard deviation
      # Log is used as it is easy to compute Log(sum(exp(values across the mean)))
      logmix, mean, logstd = tf.split(output, 3, 1)
      # Decrement step through logsumexp funtion whose usage is defined above
      logmix = logmix - tf.reduce_logsumexp(logmix, 1, keepdims=True)
      return logmix, mean, logstd
    
    # Taking mixtures of gradient, mean and standard deviation
    ...
    # A special value i.e plugged into lognormal function
    logSqrtTwoPI = np.log(np.sqrt(2.0 * np.pi))
    
    # Getting the complex logarithmic  normal function
    def tf_lognormal(y, mean, logstd):
      return -0.5 * ((y - mean) / tf.exp(logstd)) ** 2 - logstd - logSqrtTwoPI
    
    # Using the lognormal func with the log mix
    def get_lossfunc(logmix, mean, logstd, y):
      # Using the above function
      v = logmix + tf_lognormal(y, mean, logstd)
      # Using the Log(sum(exp(values around the mixture of gradients)))
      v = tf.reduce_logsumexp(v, 1, keepdims=True)
      # Reducing the Negative of mean
      return -tf.reduce_mean(v)
    
    # Flattening out the data into 1D vector of 32M values
    # This variable will be y in above implementations
    flat_target_data = tf.reshape(self.output_x,[-1, 1])
    
    # Using the Main Loss function from above
    lossfunc = get_lossfunc(out_logmix, out_mean, out_logstd, flat_target_data)
    # Reducing the mean of te loss function 
    self.cost = tf.reduce_mean(lossfunc)
    
    # If the model is trainable
    if self.hps.is_training == 1:
      
      self.lr = tf.Variable(self.hps.learning_rate, trainable=False)
      
      self.optimizer = tf.train.AdamOptimizer(self.lr)
      # Using gradient vectors instead of gradients oonly
      # these consist of grads and var
      gvs = self.optimizer.compute_gradients(self.cost)
      # Clipping of grads in self.hps.grad_clip not including the boundary values 
